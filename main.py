...
# oddělovač pro lepší čitelnost hry
ODDELOVAC = "-" * 47
...
def generovani_cisla():
    ...
    '''
    # string.digits == "0123456789"; string.digits[1:] == "123456789" → vyloučíme 0
    prvni_cislice = random.choice(string.digits[1:])
    # vytvořen set z číslic 0-9
    ostatni_cislice = list(string.digits)
    # z listu odstraněna první číslece - tedy 0
    ostatni_cislice.remove(prvni_cislice)
    # složení finálního čísla, která nezačíná 0 a má 4 číslice
...
def je_tip_validni(hracovo_cislo):
    ...
    if not hracovo_cislo.isdigit():
        return False
    if len(set(hracovo_cislo)) != 4:
        return False
    if hracovo_cislo[0] == "0":
        return False
    else:
        return True
...
def pluralize(bull_cow, word):
    '''
    Pokud je počet cows nebo bull větší než 1 nebo rovna 0 přidá "s" jako množné číslo.
    '''
    if bull_cow == 0 or bull_cow > 1:
        words = word + "s"
        return words
    else:
        return word
